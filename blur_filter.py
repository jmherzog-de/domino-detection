...
class BlurFilter(FilterBaseClass):
    ...
    def __onChange_kSizeCols(self, value:int) -> None:
        """
        Value changed event for parameter kSizeCols.
        The variable kSizeCols contains the value for the blur matrix cols size.
    ...
    def __onChange_kSizeRows(self, value):
        """
        Value changed event for parameter kSizeRows.
        THe variable kSizeRows contains the value for the blur matrix rows size.
    ...
    def __action(self):
        ...
        if self.__kSize_cols == 0 or self.__kSize_rows == 0:
            self._cvProcImage = self._cvImage.copy()
            cv2.imshow(self._windowName, self._cvImage)
        else:
            self._cvProcImage = cv2.blur(self._cvImage, (self.__kSize_cols, self.__kSize_rows) )
