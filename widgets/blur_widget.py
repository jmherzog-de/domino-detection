...
class BlurWidget(BaseWidget):
    ...
    The user has the ability to change the number of rows and columns of the blur kernel.
    """
    ...
    def Action(self) -> None:
        """
        Apply blur filter on input image.
        """
        cvInputImage:np.ndarray = self.SelectInputImage()
        self.OutputImage = cv2.blur(cvInputImage, ksize=(self.__kernelSize, self.__kernelSize))
        super().Action()
